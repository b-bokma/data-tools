...
df_download = wr.s3.read_json(
        path=f"s3://{source_bucket_name}/chargebee/customers/",
        lines=True,
        convert_dates=False,  # dont convert columns to dates
        convert_axes=False
    )

print(df_download[df_download.id == '2543'])
...
for instance in instances:
    ...
    df = df_download[df_download.entity == instance]

    print(f"Recalculating customers {instance}")
    print(df[df.id == '2543'])
    ...
    str_columns = ['id','first_name','last_name','email','company','vat_number','auto_collection','vat_number_status','taxability','locale','pii_cleared','object','card_status',
                   'preferred_currency_code','cf_group','entity','phone','invoice_notes','created_from_ip','consolidated_invoicing','cf_industry','cf_renewal_email',
                   'relationship', 'use_default_hierarchy_settings', 'parent_account_access','child_account_access', 'cf_holding_id']
